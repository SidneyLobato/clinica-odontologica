...
class RelatorioController extends BaseController {
    ...
    public function paciente(?string $p=null): void {
        ...
        if($pid){
            $s=$this->db->prepare("SELECT * FROM pacientes WHERE id=? AND empresa_id=? LIMIT 1");$s->execute([$pid,$this->empresaId]);$paciente=$s->fetch()?:null;
            if($paciente)$nome=$paciente['nome'];
        } elseif($nome){
            // Compatibilidade com links antigos que ainda usem busca por nome (texto livre)
            $s=$this->db->prepare("SELECT * FROM pacientes WHERE LOWER(nome) LIKE LOWER(?) AND empresa_id=? LIMIT 1");$s->execute(["%$nome%",$this->empresaId]);$paciente=$s->fetch()?:null;
        }

        if($paciente){
            $tc=$this->db->prepare("SELECT COUNT(ap.id) FROM atendimento_procedimentos ap JOIN atendimentos a ON ap.id_atendimento=a.id WHERE a.paciente_id=? AND a.empresa_id=?");$tc->execute([$paciente['id'],$this->empresaId]);$totalPags=(int)ceil($tc->fetchColumn()/$pp);
    ...
    public function financeiro(?string $p=null): void {
        ...
        $sb=$this->db->prepare("SELECT COALESCE(SUM(ap.valor_procedimento),0) FROM atendimentos a JOIN atendimento_procedimentos ap ON a.id=ap.id_atendimento WHERE a.data_atendimento BETWEEN ? AND ? AND a.status_pagamento='pago' AND ap.status_execucao='feito' AND a.empresa_id=?");
        $sb->execute([$ini.' 00:00:00',$fim.' 23:59:59',$this->empresaId]);$totais['bruto']=(float)$sb->fetchColumn();
        $sl=$this->db->prepare("SELECT COALESCE(SUM(valor_liquido_clinica),0) FROM atendimentos WHERE data_atendimento BETWEEN ? AND ? AND status_pagamento='pago' AND empresa_id=?");
        $sl->execute([$ini.' 00:00:00',$fim.' 23:59:59',$this->empresaId]);$totais['liquido']=(float)$sl->fetchColumn();
        // Gráfico
