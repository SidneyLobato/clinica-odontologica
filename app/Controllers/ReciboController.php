...
class ReciboController extends BaseController {
    public function index(?string $id=null): void {
        ...
        $s=$this->db->prepare("SELECT a.id,a.data_atendimento,p.nome AS paciente_nome,p.cpf AS paciente_cpf,p.endereco,p.numero,p.bairro,p.cidade,p.estado,u.nome AS dentista_nome,SUM(ap.valor_procedimento) AS valor_total FROM atendimentos a JOIN pacientes p ON a.paciente_id=p.id JOIN usuarios u ON a.id_dentista=u.id LEFT JOIN atendimento_procedimentos ap ON a.id=ap.id_atendimento WHERE a.id=? AND ap.status_execucao='feito' GROUP BY a.id");
        $s->execute([$id]);$at=$s->fetch();if(!$at){echo"<p>Recibo não encontrado.</p>";return;}
        ...
        $this->renderSemLayout('recibo/index',['at'=>$at,'procs'=>$procs,'dataFmt'=>$dataFmt]);
    }
}
