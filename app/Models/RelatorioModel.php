...
class RelatorioModel extends BaseModel {
    ...
    public function resumoDiario(string $d, ?int $did=null): array {
        ...
        $sb=$this->db->prepare("SELECT COALESCE(SUM(ap.valor_procedimento),0) FROM atendimentos a JOIN atendimento_procedimentos ap ON a.id=ap.id_atendimento WHERE DATE(a.data_atendimento)=? AND a.status_pagamento='pago' AND ap.status_execucao='feito' $f");
        $sb->execute([$d]);$bruto=(float)$sb->fetchColumn();
        $s2=$this->db->prepare("SELECT COALESCE(SUM(taxa_cartao),0),COALESCE(SUM(custo_auxiliar),0),COALESCE(SUM(comissao_dentista),0) FROM atendimentos a WHERE DATE(a.data_atendimento)=? AND a.status_pagamento='pago' AND EXISTS(SELECT 1 FROM atendimento_procedimentos ap WHERE ap.id_atendimento=a.id AND ap.status_execucao='feito') $f");
        $s2->execute([$d]);[$taxa,$custo,$com]=array_values($s2->fetch(PDO::FETCH_NUM));
        $sd=$this->db->prepare("SELECT COALESCE(SUM(valor),0) FROM despesas WHERE data_despesa=?");$sd->execute([$d]);$desp=(float)$sd->fetchColumn();
    ...
    public function pagDentistas(string $d, ?int $did=null): array {
        ...
        $s->execute([$d]);return $s->fetchAll();
    }
    public function despesasDia(string $d): array { $s=$this->db->prepare("SELECT * FROM despesas WHERE data_despesa=? ORDER BY descricao");$s->execute([$d]);return $s->fetchAll(); }
    public function resumoMensal(string $ini, string $fim, ?int $did=null): array {
        ...
        $s->execute(['ini'=>$ini.' 00:00:00','fim'=>$fim.' 23:59:59']);return $s->fetchAll();
    }
    public function procedimentos(string $ini, string $fim): array {
        ...
        $s->execute([$ini.' 00:00:00',$fim.' 23:59:59']);return $s->fetchAll();
    }
    public function financeiroGeral(string $ini, string $fim, int $pg, int $pp): array {
        ...
        $s->bindValue(':ini',$ini);$s->bindValue(':fim',$fim);$s->bindValue(':l',$pp,PDO::PARAM_INT);$s->bindValue(':o',$off,PDO::PARAM_INT);$s->execute();return $s->fetchAll();
    }
    public function financeiroTotais(string $ini, string $fim): array {
        ...
        $s->execute(['ini'=>$ini,'fim'=>$fim]);return $s->fetch();
    }
    public function contarPago(string $ini, string $fim): int { $s=$this->db->prepare("SELECT COUNT(DISTINCT id) FROM atendimentos WHERE DATE(data_atendimento) BETWEEN :ini AND :fim AND status_pagamento='pago'");$s->execute(['ini'=>$ini,'fim'=>$fim]);return(int)$s->fetchColumn(); }
    public function historicoPac(int $pid, int $pg, int $pp): array {
        $off=($pg-1)*$pp;
        $s=$this->db->prepare("SELECT ap.id AS ap_id,proc.nome AS procedimento_nome,ap.local,ap.descricao,ap.status_execucao,a.data_atendimento,a.status_pagamento,ap.url_arquivo FROM atendimento_procedimentos ap JOIN atendimentos a ON ap.id_atendimento=a.id JOIN procedimentos proc ON ap.id_procedimento=proc.id WHERE a.paciente_id=:pid ORDER BY a.data_atendimento DESC LIMIT :l OFFSET :o");
        $s->bindValue(':pid',$pid,PDO::PARAM_INT);$s->bindValue(':l',$pp,PDO::PARAM_INT);$s->bindValue(':o',$off,PDO::PARAM_INT);$s->execute();return $s->fetchAll();
    }
    public function statusDentes(int $pid): array {
        $s=$this->db->prepare("SELECT ap.local,ap.status_execucao FROM atendimento_procedimentos ap JOIN atendimentos a ON ap.id_atendimento=a.id WHERE a.paciente_id=? AND ap.local IS NOT NULL AND ap.local!=''");
        $s->execute([$pid]);return $s->fetchAll();
    }
    public function grafico(string $ini, string $fim): array {
        $s=$this->db->prepare("SELECT dia,SUM(fat) AS faturamento,SUM(desp) AS despesa FROM(SELECT DATE(a.data_atendimento) AS dia,SUM(ap.valor_procedimento) AS fat,0 AS desp FROM atendimentos a JOIN atendimento_procedimentos ap ON a.id=ap.id_atendimento WHERE a.data_atendimento BETWEEN ? AND ? AND a.status_pagamento='pago' AND ap.status_execucao='feito' GROUP BY DATE(a.data_atendimento) UNION ALL SELECT DATE(data_despesa),0,SUM(valor) FROM despesas WHERE data_despesa BETWEEN ? AND ? GROUP BY DATE(data_despesa))T GROUP BY dia ORDER BY dia");
        $s->execute([$ini.' 00:00:00',$fim.' 23:59:59',$ini,$fim]);return $s->fetchAll();
    }
    public function distPagamentos(string $ini, string $fim): array {
        $s=$this->db->prepare("SELECT ap.forma_pagamento,SUM(ap.valor) AS total FROM atendimento_pagamentos ap JOIN atendimentos a ON ap.id_atendimento=a.id WHERE a.data_atendimento BETWEEN ? AND ? GROUP BY ap.forma_pagamento");
        $s->execute([$ini.' 00:00:00',$fim.' 23:59:59']);return $s->fetchAll(PDO::FETCH_KEY_PAIR);
    }
}
