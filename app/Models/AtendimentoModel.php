<?php
/**
 * AtendimentoModel — Multi-empresa
 *
...
class AtendimentoModel extends BaseModel {
    ...
    public function salvarCompleto(array $post, ?string $urlArquivo, int $pacienteId): array {
        ...
        try {
            ...
            // Faturamento mensal para regra de comissão (só desta clínica)
            $ini = date('Y-m-01 00:00:00'); $fim = date('Y-m-t 23:59:59');
            $sf  = $this->db->prepare(
                "SELECT COALESCE(SUM(ap.valor_procedimento),0)
                   FROM atendimento_procedimentos ap JOIN atendimentos a ON ap.id_atendimento=a.id
                  WHERE a.data_atendimento BETWEEN ? AND ? AND a.empresa_id=?"
            );
            $sf->execute([$ini, $fim, $this->empresaId]); $fatMensal = (float)$sf->fetchColumn();

            $pagosAgora = [];
            $pendentes  = [];
            // Procedimento precisa pertencer à empresa atual
            $sp = $this->db->prepare("SELECT id,nome,categoria,valor_base FROM procedimentos WHERE id=? AND empresa_id=?");

            foreach (($procsIn['id'] ?? []) as $k => $procId) {
                $qty = (int)($procsIn['quantidade'][$k] ?? 1);
                if (!$procId || $qty <= 0) continue;
                $sp->execute([$procId, $this->empresaId]); $proc = $sp->fetch();
            ...
            if (!empty($pagosAgora)) {
                ...
                $this->db->prepare(
                    "UPDATE atendimentos SET valor_total=?,comissao_dentista=?,custo_auxiliar=?,valor_liquido_clinica=?,status_pagamento='pago',taxa_cartao=0,id_dentista_especialista=?,id_dentista_vendedor=? WHERE id=? AND empresa_id=?"
                )->execute([$valorBruto,$totComissao,$totCusto,$valLiq,$idEspec,$idVend,$idPrincipal,$this->empresaId]);
            ...
            if (!empty($pendentes)) {
                ...
                $valLiq = $valorBruto - $totComissao - $totCusto;
                $this->db->prepare(
                    "UPDATE atendimentos SET valor_total=?,comissao_dentista=?,custo_auxiliar=?,valor_liquido_clinica=?,status_pagamento='pendente',id_dentista_especialista=?,id_dentista_vendedor=? WHERE id=? AND empresa_id=?"
                )->execute([$valorBruto,$totComissao,$totCusto,$valLiq,$idEspec,$idVend,$idPend,$this->empresaId]);
    ...
    /** Registra pagamentos para atendimentos pagos na hora (Finalizados) */
    public function registrarPagamentosFinalizados(int $atendId, array $pags, FinanceiroModel $fin): void {
        ...
        $this->db->beginTransaction();
        try {
            $spag = $this->db->prepare("INSERT INTO atendimento_pagamentos(id_atendimento,forma_pagamento,valor,qtd_parcelas,bandeira_id,taxa_snapshot,repassado_cliente)VALUES(?,?,?,?,?,?,?)");
            $totalTaxa = 0.0;
            ...
            foreach (($pags['forma'] ?? []) as $k => $forma) {
                ...
                if ($val <= 0) continue;
                // Para débito: se o checkbox "repassar ao cliente" não foi marcado, a taxa não é descontada
                $repassar = $forma !== 'debito' || !isset($pags['repassar_debito'][$k]) || $pags['repassar_debito'][$k] != '0';

                $res = $fin->calcularLiquidoMaquininha($val, $forma, $parc, $bid, $repassar);
                $totalTaxa += $res['valor_taxa'];
                $spag->execute([$atendId, $forma, $val, $parc, $bid > 0 ? $bid : null, $res['taxa_aplicada_percentual'], $repassar ? 1 : 0]);
            }

            $novoLiq = round((float)$at['valor_total'] - (float)$at['comissao_dentista'] - (float)$at['custo_auxiliar'] - $totalTaxa, 2);
            $this->db->prepare("UPDATE atendimentos SET taxa_cartao=?, valor_liquido_clinica=? WHERE id=? AND empresa_id=?")
                ->execute([$totalTaxa, $novoLiq, $atendId, $this->empresaId]);
    ...
    public function confirmarPagamento(int $atendId, array $pags, FinanceiroModel $fin): void {
        ...
        try {
            ...
            $ini = date('Y-m-01 00:00:00'); $fim = date('Y-m-t 23:59:59');
            $sf  = $this->db->prepare(
                "SELECT COALESCE(SUM(ap.valor_procedimento),0) FROM atendimento_procedimentos ap JOIN atendimentos a ON ap.id_atendimento=a.id
                 WHERE a.data_atendimento BETWEEN ? AND ? AND a.status_pagamento='pago' AND ap.status_execucao='feito' AND a.empresa_id=?"
            );
            $sf->execute([$ini,$fim,$this->empresaId]); $fatMensal = (float)$sf->fetchColumn();
            ...
            foreach (($pags['forma'] ?? []) as $k => $forma) {
                ...
                $res = $fin->calcularLiquidoMaquininha($val, $forma, $parc, $bid, $repassar);
                $totalTaxa += $res['valor_taxa'];
                $spag->execute([$atendId,$forma,$val,$parc,$bid?:null,$res['taxa_aplicada_percentual'],$repassar?1:0]);
            }
            ...
            $this->db->prepare("UPDATE atendimentos SET status_pagamento='pago',taxa_cartao=?,comissao_dentista=?,valor_liquido_clinica=? WHERE id=? AND empresa_id=?")
                ->execute([$totalTaxa,$novaComissao,$valLiq,$atendId,$this->empresaId]);
    ...
    public function pendentes(): array {
        [$where, $params] = $this->filtro("a.status_pagamento = 'pendente'");
        // filtro() usa nome de tabela sem alias — ajustamos manualmente aqui
        $where = str_replace('empresa_id', 'a.empresa_id', $where);
        $s = $this->db->prepare(
            "SELECT a.id, a.data_atendimento, p.nome AS paciente_nome, a.valor_total
               FROM atendimentos a JOIN pacientes p ON a.paciente_id = p.id
              $where
    ...
    public function procsPendentes(int $atendId): array {
        // Garante que o atendimento pertence à empresa atual antes de listar
        if ($this->findById($atendId) === null) return [];
        $s = $this->db->prepare(
            "SELECT ap.id,proc.nome AS procedimento_nome,ap.local,ap.valor_procedimento,ap.status_execucao,ap.url_arquivo
               FROM atendimento_procedimentos ap JOIN procedimentos proc ON ap.id_procedimento=proc.id
              WHERE ap.id_atendimento=? AND ap.status_execucao IN('pendente','finalizado')"
        );
        $s->execute([$atendId]);
        return $s->fetchAll();
    }
