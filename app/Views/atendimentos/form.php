...
$(function(){
    ...
    function criarLinhaPagamento(){
        ...
        row.style.cssText='display:grid;grid-template-columns:160px 110px 180px 90px auto;gap:.5rem;align-items:center;margin-bottom:.5rem;';
        ...
        const sb=document.createElement('select');sb.style.display='none';sb.name='pagamento[bandeira_id][]';sb.style.padding='6px';
        ...
        const sp=document.createElement('select');sp.style.display='none';sp.name='pagamento[parcelas][]';sp.style.padding='6px';
        ...
        sf.addEventListener('change',()=>{
            ...
            sb.style.display=isCard?'block':'none';
            sp.style.display=sf.value==='credito'?'block':'none';
            if(!isCard){sb.value='';sp.value='1';}
        });
        val.addEventListener('input',updatePagTotal);
        rm.addEventListener('click',()=>{row.remove();updatePagTotal();});

        row.append(sf,val,sb,sp,rm);
    ...
    function updatePagTotal(){
        const totalFin=adicionados.filter(p=>p.status==='finalizado').reduce((s,p)=>s+p.valor,0);
        let pago=0;
        document.querySelectorAll('.pag-row-form input[type="number"]').forEach(i=>pago+=parseFloat(i.value)||0);
    ...
    $('#form-atendimento').on('submit',async function(e){
        ...
        if(totalFin>0){
            ...
            document.querySelectorAll('.pag-row-form').forEach(row=>{
                ...
                const bid=sels[1]?.style.display!=='none'?sels[1].value:'';
                pagTotal+=val;
